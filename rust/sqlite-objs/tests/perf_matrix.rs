    ...
    fn new() -> Result<Self> {
        ...
        match mode {
            ...
            PerfMode::File => {
                ...
                Self::configure_pragmas(&conn, mode)?;
                Ok(TestDb { conn, temp_dir: Some(temp_dir) })
            }
            PerfMode::Azure => {
                load_dotenv();
                ...
                eprintln!("[perf_matrix] AZURE: account={account} container={container} blob={blob_name}");
...
impl TestDb {
    fn run_work_loop(&self) {
        ...
        // Create work table
        self.execute("CREATE TABLE IF NOT EXISTS _perf_work (
        ...
        let count: i64 = self.query_row(
            "SELECT COUNT(*) FROM _perf_work", [], |r| r.get(0)
        ).unwrap();
        assert_eq!(count, n as i64);

        // Group-by query
        let cat_count: i64 = self.query_row(
            "SELECT COUNT(DISTINCT category) FROM _perf_work", [], |r| r.get(0)
        ).unwrap();
        assert!(cat_count > 0);
        ...
        // Range query
        let sum: f64 = self.query_row(
            "SELECT SUM(value) FROM _perf_work WHERE id BETWEEN 10 AND 100", [], |r| r.get(0)
        ).unwrap_or(0.0);
        assert!(sum >= 0.0);

        // Delete subset
        self.execute(
            "DELETE FROM _perf_work WHERE id < ?", [n as i64 / 4]
        ).unwrap();

        // Verify remaining
        let remaining: i64 = self.query_row(
            "SELECT COUNT(*) FROM _perf_work", [], |r| r.get(0)
        ).unwrap();
        assert!(remaining > 0);
    }
...
schema_test!(schema_simple_table, 
...
schema_test!(schema_multi_column,
    ...
    |db: &TestDb| {
        db.execute("INSERT INTO products (sku, name, price, quantity) VALUES (?, ?, ?, ?)",
            ["ABC123", "Widget", "9.99", "100"]).unwrap();
        let count: i64 = db.query_row("SELECT COUNT(*) FROM products", [], |r| r.get(0)).unwrap();
        assert_eq!(count, 1);
    }
);

schema_test!(schema_with_index,
...
schema_test!(schema_unique_constraint,
    "CREATE TABLE emails (id INTEGER PRIMARY KEY, address TEXT UNIQUE);",
    |db: &TestDb| {
        db.execute("INSERT INTO emails (address) VALUES (?)", ["test@example.com"]).unwrap();
        let result = db.execute("INSERT INTO emails (address) VALUES (?)", ["test@example.com"]);
...
schema_test!(schema_check_constraint,
    "CREATE TABLE inventory (id INTEGER PRIMARY KEY, quantity INTEGER CHECK(quantity >= 0));",
    |db: &TestDb| {
        db.execute("INSERT INTO inventory (quantity) VALUES (10)", []).unwrap();
        let result = db.execute("INSERT INTO inventory (quantity) VALUES (-5)", []);
        assert!(result.is_err()); // Check constraint violation
    }
);

schema_test!(schema_default_values,
    "CREATE TABLE settings (key TEXT PRIMARY KEY, value TEXT, enabled INTEGER DEFAULT 1);",
    |db: &TestDb| {
        db.execute("INSERT INTO settings (key, value) VALUES ('timeout', '30')", []).unwrap();
        let enabled: i64 = db.query_row("SELECT enabled FROM settings WHERE key='timeout'", [], |r| r.get(0)).unwrap();
        assert_eq!(enabled, 1);
    }
);

schema_test!(schema_drop_table,
...
schema_test!(schema_multiple_indexes,
...
schema_test!(schema_blob_column, "CREATE TABLE files (id INTEGER PRIMARY KEY, data BLOB);", |_db: &TestDb| {});
schema_test!(schema_text_affinity, "CREATE TABLE docs (id INTEGER PRIMARY KEY, content TEXT);", |_db: &TestDb| {});
schema_test!(schema_real_affinity, "CREATE TABLE measurements (id INTEGER PRIMARY KEY, value REAL);", |_db: &TestDb| {});
schema_test!(schema_integer_affinity, "CREATE TABLE counters (id INTEGER PRIMARY KEY, count INTEGER);", |_db: &TestDb| {});
schema_test!(schema_numeric_affinity, "CREATE TABLE stats (id INTEGER PRIMARY KEY, amount NUMERIC);", |_db: &TestDb| {});
schema_test!(schema_not_null, "CREATE TABLE required (id INTEGER PRIMARY KEY, name TEXT NOT NULL);", |_db: &TestDb| {});
schema_test!(schema_autoincrement, "CREATE TABLE seq (id INTEGER PRIMARY KEY AUTOINCREMENT, val TEXT);", |_db: &TestDb| {});
schema_test!(schema_without_rowid, "CREATE TABLE kvstore (key TEXT PRIMARY KEY, value TEXT) WITHOUT ROWID;", |_db: &TestDb| {});
schema_test!(schema_view, "CREATE TABLE base (id INTEGER); CREATE VIEW v AS SELECT * FROM base;", |_db: &TestDb| {});
schema_test!(schema_trigger, "CREATE TABLE audit (id INTEGER); CREATE TRIGGER t AFTER INSERT ON audit BEGIN SELECT 1; END;", |_db: &TestDb| {});
...
schema_test!(schema_expression_index, "CREATE TABLE names (id INTEGER, name TEXT); CREATE INDEX idx_lower ON names(LOWER(name));", |_db: &TestDb| {});
schema_test!(schema_drop_index, "CREATE TABLE t (id INTEGER); CREATE INDEX i ON t(id); DROP INDEX i;", |_db: &TestDb| {});
schema_test!(schema_temp_table, "CREATE TEMP TABLE session (key TEXT, value TEXT);", |_db: &TestDb| {});
schema_test!(schema_if_not_exists, "CREATE TABLE IF NOT EXISTS users (id INTEGER); CREATE TABLE IF NOT EXISTS users (id INTEGER);", |_db: &TestDb| {});
schema_test!(schema_multiple_tables, "CREATE TABLE t1 (id INTEGER); CREATE TABLE t2 (id INTEGER); CREATE TABLE t3 (id INTEGER);", |_db: &TestDb| {});
...
schema_test!(schema_collate, "CREATE TABLE names (id INTEGER, name TEXT COLLATE NOCASE);", |_db: &TestDb| {});
schema_test!(schema_generated_column, "CREATE TABLE calc (a INTEGER, b INTEGER, sum INTEGER GENERATED ALWAYS AS (a+b));", |_db: &TestDb| {});
schema_test!(schema_strict_table, "CREATE TABLE strict_data (id INTEGER PRIMARY KEY, val TEXT) STRICT;", |_db: &TestDb| {});
...
single_row_test!(row_update, |db: &TestDb| {
    ...
    db.execute("INSERT INTO t (val) VALUES ('old')", []).unwrap();
...
single_row_test!(row_delete, |db: &TestDb| {
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, val TEXT)", []).unwrap();
    db.execute("INSERT INTO t (val) VALUES ('data')", []).unwrap();
...
single_row_test!(row_select_by_id, |db: &TestDb| {
    ...
    let name: String = db.query_row("SELECT name FROM t WHERE id=2", [], |r| r.get(0)).unwrap();
    assert_eq!(name, "Bob");
});

single_row_test!(row_update_multiple_columns, |db: &TestDb| {
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, a TEXT, b INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (1, 'x', 10)", []).unwrap();
    db.execute("UPDATE t SET a='y', b=20 WHERE id=1", []).unwrap();
    let (a, b): (String, i64) = db.query_row("SELECT a, b FROM t WHERE id=1", [], |r| Ok((r.get(0)?, r.get(1)?))).unwrap();
    assert_eq!((a.as_str(), b), ("y", 20));
});

single_row_test!(row_insert_returning, |db: &TestDb| {
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, val TEXT)", []).unwrap();
    db.execute("INSERT INTO t (val) VALUES ('test')", []).unwrap();
...
single_row_test!(row_insert_unicode, |db: &TestDb| {
    db.execute("CREATE TABLE t (val TEXT)", []).unwrap();
    db.execute("INSERT INTO t VALUES ('Hello 世界 🌍')", []).unwrap();
    let val: String = db.query_row("SELECT val FROM t", [], |r| r.get(0)).unwrap();
...
single_row_test!(row_update_where_clause, |db: &TestDb| {
    ...
    let val: i64 = db.query_row("SELECT val FROM t WHERE id=2", [], |r| r.get(0)).unwrap();
    assert_eq!(val, 25);
});
...
single_row_test!(row_update_all, |db: &TestDb| {
    ...
    let sum: i64 = db.query_row("SELECT SUM(val) FROM t", [], |r| r.get(0)).unwrap();
    assert_eq!(sum, 297);
});
...
single_row_test!(row_select_star, |db: &TestDb| {
    db.execute("CREATE TABLE t (a INTEGER, b TEXT, c REAL)", []).unwrap();
    db.execute("INSERT INTO t VALUES (1, 'x', 2.5)", []).unwrap();
    let (a, b, c): (i64, String, f64) = db.query_row("SELECT * FROM t", [], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?))).unwrap();
...
single_row_test!(row_select_expressions, |db: &TestDb| {
    ...
    let sum: i64 = db.query_row("SELECT a + b FROM t", [], |r| r.get(0)).unwrap();
    assert_eq!(sum, 15);
});
...
single_row_test!(row_insert_or_replace, |db: &TestDb| {
    ...
    db.execute("INSERT OR REPLACE INTO t VALUES (1, 'second')", []).unwrap();
    let val: String = db.query_row("SELECT val FROM t WHERE id=1", [], |r| r.get(0)).unwrap();
...
single_row_test!(row_upsert, |db: &TestDb| {
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, val TEXT)", []).unwrap();
    db.execute("INSERT INTO t VALUES (1, 'initial') ON CONFLICT(id) DO UPDATE SET val='updated'", []).unwrap();
    db.execute("INSERT INTO t VALUES (1, 'new') ON CONFLICT(id) DO UPDATE SET val='updated'", []).unwrap();
    let val: String = db.query_row("SELECT val FROM t WHERE id=1", [], |r| r.get(0)).unwrap();
...
single_row_test!(row_select_case, |db: &TestDb| {
    ...
    let label: String = db.query_row("SELECT CASE WHEN val > 3 THEN 'high' ELSE 'low' END FROM t", [], |r| r.get(0)).unwrap();
...
single_row_test!(row_select_coalesce, |db: &TestDb| {
    db.execute("CREATE TABLE t (val TEXT)", []).unwrap();
    db.execute("INSERT INTO t VALUES (NULL)", []).unwrap();
    let val: String = db.query_row("SELECT COALESCE(val, 'default') FROM t", [], |r| r.get(0)).unwrap();
...
single_row_test!(row_select_substr, |db: &TestDb| {
    ...
    let sub: String = db.query_row("SELECT SUBSTR(val, 1, 5) FROM t", [], |r| r.get(0)).unwrap();
    assert_eq!(sub, "hello");
});
...
single_row_test!(row_select_length, |db: &TestDb| {
    ...
    let len: i64 = db.query_row("SELECT LENGTH(val) FROM t", [], |r| r.get(0)).unwrap();
    assert_eq!(len, 4);
});
...
single_row_test!(row_select_upper, |db: &TestDb| {
    db.execute("CREATE TABLE t (val TEXT)", []).unwrap();
    db.execute("INSERT INTO t VALUES ('hello')", []).unwrap();
    let upper: String = db.query_row("SELECT UPPER(val) FROM t", [], |r| r.get(0)).unwrap();
...
single_row_test!(row_select_lower, |db: &TestDb| {
    ...
    let lower: String = db.query_row("SELECT LOWER(val) FROM t", [], |r| r.get(0)).unwrap();
    assert_eq!(lower, "world");
});

single_row_test!(row_select_trim, |db: &TestDb| {
    db.execute("CREATE TABLE t (val TEXT)", []).unwrap();
    db.execute("INSERT INTO t VALUES ('  spaces  ')", []).unwrap();
    let trimmed: String = db.query_row("SELECT TRIM(val) FROM t", [], |r| r.get(0)).unwrap();
...
single_row_test!(row_select_round, |db: &TestDb| {
    db.execute("CREATE TABLE t (val REAL)", []).unwrap();
    db.execute("INSERT INTO t VALUES (3.14159)", []).unwrap();
    let rounded: f64 = db.query_row("SELECT ROUND(val, 2) FROM t", [], |r| r.get(0)).unwrap();
...
single_row_test!(row_select_abs, |db: &TestDb| {
    db.execute("CREATE TABLE t (val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (-42)", []).unwrap();
    let abs_val: i64 = db.query_row("SELECT ABS(val) FROM t", [], |r| r.get(0)).unwrap();
    assert_eq!(abs_val, 42);
});

single_row_test!(row_select_datetime, |db: &TestDb| {
    let now: String = db.query_row("SELECT DATETIME('now')", [], |r| r.get(0)).unwrap();
...
bulk_test!(bulk_insert_100_rows, |db: &TestDb| {
    db.execute("CREATE TABLE t (id INTEGER, val TEXT)", []).unwrap();
    for i in 0..100 {
        db.execute("INSERT INTO t VALUES (?, ?)", rusqlite::params![i, format!("row{}", i)]).unwrap();
    }
    let count: i64 = db.query_row("SELECT COUNT(*) FROM t", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 100);
});
...
bulk_test!(bulk_select_where_in, |db: &TestDb| {
    ...
    let count: i64 = db.query_row("SELECT COUNT(*) FROM t WHERE id IN (2, 4, 5)", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 3);
});

bulk_test!(bulk_select_like, |db: &TestDb| {
    db.execute("CREATE TABLE t (name TEXT)", []).unwrap();
    db.execute("INSERT INTO t VALUES ('apple'), ('application'), ('banana'), ('apricot')", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM t WHERE name LIKE 'app%'", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 2);
});
...
bulk_test!(bulk_aggregate_avg, |db: &TestDb| {
    db.execute("CREATE TABLE t (val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (10), (20), (30), (40), (50)", []).unwrap();
    let avg: f64 = db.query_row("SELECT AVG(val) FROM t", [], |r| r.get(0)).unwrap();
    assert!((avg - 30.0).abs() < 0.01);
});

bulk_test!(bulk_aggregate_min_max, |db: &TestDb| {
    db.execute("CREATE TABLE t (val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (5), (15), (3), (22), (8)", []).unwrap();
    let (min, max): (i64, i64) = db.query_row("SELECT MIN(val), MAX(val) FROM t", [], |r| Ok((r.get(0)?, r.get(1)?))).unwrap();
    assert_eq!((min, max), (3, 22));
});

bulk_test!(bulk_group_by, |db: &TestDb| {
    db.execute("CREATE TABLE t (category TEXT, amount INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES ('A', 10), ('B', 20), ('A', 15), ('B', 25), ('A', 5)", []).unwrap();
    let sum_a: i64 = db.query_row("SELECT SUM(amount) FROM t WHERE category='A' GROUP BY category", [], |r| r.get(0)).unwrap();
    assert_eq!(sum_a, 30);
});

bulk_test!(bulk_order_by_asc, |db: &TestDb| {
    db.execute("CREATE TABLE t (val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (30), (10), (20), (50), (40)", []).unwrap();
    let first: i64 = db.query_row("SELECT val FROM t ORDER BY val ASC LIMIT 1", [], |r| r.get(0)).unwrap();
    assert_eq!(first, 10);
});
...
bulk_test!(bulk_order_by_desc, |db: &TestDb| {
    db.execute("CREATE TABLE t (val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (30), (10), (20), (50), (40)", []).unwrap();
    let first: i64 = db.query_row("SELECT val FROM t ORDER BY val DESC LIMIT 1", [], |r| r.get(0)).unwrap();
    assert_eq!(first, 50);
});
...
bulk_test!(bulk_limit_offset, |db: &TestDb| {
    ...
    let val: i64 = db.query_row("SELECT id FROM t ORDER BY id LIMIT 1 OFFSET 10", [], |r| r.get(0)).unwrap();
    assert_eq!(val, 11);
});
...
bulk_test!(bulk_update_batch, |db: &TestDb| {
    ...
    let count: i64 = db.query_row("SELECT COUNT(*) FROM t WHERE val=1", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 50);
});
...
bulk_test!(bulk_select_distinct, |db: &TestDb| {
    db.execute("CREATE TABLE t (val TEXT)", []).unwrap();
    db.execute("INSERT INTO t VALUES ('a'), ('b'), ('a'), ('c'), ('b'), ('a')", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(DISTINCT val) FROM t", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 3);
});

bulk_test!(bulk_select_having, |db: &TestDb| {
    db.execute("CREATE TABLE t (category TEXT, amount INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES ('A', 10), ('B', 5), ('A', 20), ('C', 3), ('B', 8)", []).unwrap();
...
bulk_test!(bulk_aggregate_group_by_multiple, |db: &TestDb| {
    db.execute("CREATE TABLE t (a TEXT, b TEXT, val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES ('x', 'y', 10), ('x', 'z', 20), ('x', 'y', 30), ('w', 'y', 40)", []).unwrap();
    let sum: i64 = db.query_row("SELECT SUM(val) FROM t WHERE a='x' AND b='y' GROUP BY a, b", [], |r| r.get(0)).unwrap();
    assert_eq!(sum, 40);
});
...
bulk_test!(bulk_select_between, |db: &TestDb| {
    ...
    let count: i64 = db.query_row("SELECT COUNT(*) FROM t WHERE val BETWEEN 25 AND 75", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 51);
});
...
bulk_test!(bulk_select_not_in, |db: &TestDb| {
    db.execute("CREATE TABLE t (id INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (1), (2), (3), (4), (5)", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM t WHERE id NOT IN (2, 4)", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 3);
});

bulk_test!(bulk_select_glob, |db: &TestDb| {
    db.execute("CREATE TABLE t (name TEXT)", []).unwrap();
    db.execute("INSERT INTO t VALUES ('test1'), ('test2'), ('other'), ('test3')", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM t WHERE name GLOB 'test*'", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 3);
});
...
bulk_test!(bulk_select_order_multiple, |db: &TestDb| {
    db.execute("CREATE TABLE t (a INTEGER, b INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (1, 20), (2, 10), (1, 30), (2, 5)", []).unwrap();
    let (a, b): (i64, i64) = db.query_row("SELECT a, b FROM t ORDER BY a ASC, b DESC LIMIT 1", [], |r| Ok((r.get(0)?, r.get(1)?))).unwrap();
    assert_eq!((a, b), (1, 30));
});

bulk_test!(bulk_aggregate_total, |db: &TestDb| {
    db.execute("CREATE TABLE t (val REAL)", []).unwrap();
    db.execute("INSERT INTO t VALUES (NULL), (10.5), (20.3), (NULL), (15.2)", []).unwrap();
    let total: f64 = db.query_row("SELECT TOTAL(val) FROM t", [], |r| r.get(0)).unwrap();
    assert!((total - 46.0).abs() < 0.1);
});

bulk_test!(bulk_select_null_handling, |db: &TestDb| {
    db.execute("CREATE TABLE t (val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (1), (NULL), (2), (NULL), (3)", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM t WHERE val IS NOT NULL", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 3);
});
...
bulk_test!(bulk_string_concatenation, |db: &TestDb| {
    ...
    let name: String = db.query_row("SELECT first || ' ' || last FROM t WHERE first='John'", [], |r| r.get(0)).unwrap();
    assert_eq!(name, "John Doe");
});

bulk_test!(bulk_math_operations, |db: &TestDb| {
    db.execute("CREATE TABLE t (a INTEGER, b INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (10, 3), (20, 4), (15, 5)", []).unwrap();
    let result: i64 = db.query_row("SELECT SUM(a * b) FROM t", [], |r| r.get(0)).unwrap();
...
bulk_test!(bulk_select_random, |db: &TestDb| {
    ...
    let val: i64 = db.query_row("SELECT id FROM t ORDER BY RANDOM() LIMIT 1", [], |r| r.get(0)).unwrap();
...
bulk_test!(bulk_insert_select, |db: &TestDb| {
    ...
    db.execute("INSERT INTO dest SELECT id FROM source WHERE id < 25", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM dest", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 25);
});
...
bulk_test!(bulk_update_from_select, |db: &TestDb| {
    ...
    db.execute("UPDATE t SET val=(SELECT COUNT(*) FROM t) WHERE id=0", []).unwrap();
    let val: i64 = db.query_row("SELECT val FROM t WHERE id=0", [], |r| r.get(0)).unwrap();
    assert_eq!(val, 100);
});
...
bulk_test!(bulk_aggregate_stddev_placeholder, |db: &TestDb| {
    ...
    let avg: f64 = db.query_row("SELECT AVG(val * val) FROM t", [], |r| r.get(0)).unwrap();
    assert!(avg > 0.0);
});

bulk_test!(bulk_json_extract, |db: &TestDb| {
    db.execute("CREATE TABLE t (data TEXT)", []).unwrap();
    db.execute(r#"INSERT INTO t VALUES ('{"name":"Alice","age":30}')"#, []).unwrap();
    let name: String = db.query_row("SELECT JSON_EXTRACT(data, '$.name') FROM t", [], |r| r.get(0)).unwrap();
...
bulk_test!(bulk_fts5_search, |db: &TestDb| {
    ...
    let count: i64 = db.query_row("SELECT COUNT(*) FROM docs WHERE docs MATCH 'SQLite'", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 2);
});
...
bulk_test!(bulk_window_function, |db: &TestDb| {
    db.execute("CREATE TABLE t (id INTEGER, val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (1, 10), (2, 20), (3, 30)", []).unwrap();
    let sum: i64 = db.query_row("SELECT SUM(val) OVER () FROM t LIMIT 1", [], |r| r.get(0)).unwrap();
    assert_eq!(sum, 60);
});

bulk_test!(bulk_rank_window, |db: &TestDb| {
    db.execute("CREATE TABLE t (name TEXT, score INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES ('A', 90), ('B', 80), ('C', 90), ('D', 70)", []).unwrap();
    let rank: i64 = db.query_row(
...
bulk_test!(bulk_row_number, |db: &TestDb| {
    db.execute("CREATE TABLE t (val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (100), (200), (300)", []).unwrap();
    let row_num: i64 = db.query_row(
...
bulk_test!(bulk_partition_by, |db: &TestDb| {
    db.execute("CREATE TABLE t (category TEXT, val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES ('A', 10), ('A', 20), ('B', 30), ('B', 40)", []).unwrap();
    let sum: i64 = db.query_row("SELECT SUM(val) OVER (PARTITION BY category) FROM t WHERE category='A' LIMIT 1", [], |r| r.get(0)).unwrap();
    assert_eq!(sum, 30);
});
...
txn_test!(txn_conflict_resolution, |db: &TestDb| {
    ...
    db.execute("BEGIN", []).unwrap();
    db.execute("INSERT OR REPLACE INTO t VALUES (1, 200)", []).unwrap();
    db.execute("COMMIT", []).unwrap();
    let val: i64 = db.query_row("SELECT val FROM t WHERE id=1", [], |r| r.get(0)).unwrap();
    assert_eq!(val, 200);
});
...
txn_test!(txn_implicit_commit_on_ddl, |db: &TestDb| {
    ...
    db.execute("COMMIT", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM t1", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 1);
});
...
txn_test!(txn_integrity_check, |db: &TestDb| {
    db.execute("CREATE TABLE t (val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (1), (2), (3)", []).unwrap();
    let result: String = db.query_row("PRAGMA integrity_check", [], |r| r.get(0)).unwrap();
    assert_eq!(result, "ok");
});
...
txn_test!(txn_foreign_key_violation_rollback, |db: &TestDb| {
    ...
    db.execute("CREATE TABLE c (id INTEGER, pid INTEGER REFERENCES p(id))", []).unwrap();
    db.execute("BEGIN", []).unwrap();
    ...
    let count: i64 = db.query_row("SELECT COUNT(*) FROM p", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 0);
});

txn_test!(txn_check_constraint_rollback, |db: &TestDb| {
    db.execute("CREATE TABLE t (val INTEGER CHECK(val > 0))", []).unwrap();
    db.execute("BEGIN", []).unwrap();
    db.execute("INSERT INTO t VALUES (5)", []).unwrap();
...
complex_test!(complex_inner_join, |db: &TestDb| {
    ...
    db.execute("CREATE TABLE orders (id INTEGER, user_id INTEGER, amount REAL)", []).unwrap();
    db.execute("INSERT INTO users VALUES (1, 'Alice'), (2, 'Bob')", []).unwrap();
    db.execute("INSERT INTO orders VALUES (1, 1, 100.0), (2, 1, 150.0), (3, 2, 200.0)", []).unwrap();
...
complex_test!(complex_left_join, |db: &TestDb| {
    ...
    db.execute("INSERT INTO users VALUES (1, 'Alice'), (2, 'Bob'), (3, 'Charlie')", []).unwrap();
    db.execute("INSERT INTO orders VALUES (1, 1), (2, 1), (3, 2)", []).unwrap();
    let count: i64 = db.query_row(
...
complex_test!(complex_subquery_in_select, |db: &TestDb| {
    db.execute("CREATE TABLE products (id INTEGER, price REAL)", []).unwrap();
    db.execute("INSERT INTO products VALUES (1, 10.0), (2, 20.0), (3, 15.0)", []).unwrap();
    let _avg: f64 = db.query_row("SELECT AVG(price) FROM products", [], |r| r.get(0)).unwrap();
    let count: i64 = db.query_row(
...
complex_test!(complex_subquery_in_from, |db: &TestDb| {
    db.execute("CREATE TABLE sales (product TEXT, amount REAL)", []).unwrap();
    db.execute("INSERT INTO sales VALUES ('A', 100), ('B', 200), ('A', 150), ('C', 50)", []).unwrap();
...
complex_test!(complex_cte, |db: &TestDb| {
    ...
    db.execute("INSERT INTO employees VALUES (1, 'CEO', NULL), (2, 'VP', 1), (3, 'Engineer', 2)", []).unwrap();
    let count: i64 = db.query_row(
...
complex_test!(complex_union, |db: &TestDb| {
    ...
    db.execute("INSERT INTO t2 VALUES (3), (4), (5)", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM (SELECT val FROM t1 UNION SELECT val FROM t2)", [], |r| r.get(0)).unwrap();
...
complex_test!(complex_union_all, |db: &TestDb| {
    ...
    let count: i64 = db.query_row("SELECT COUNT(*) FROM (SELECT val FROM t1 UNION ALL SELECT val FROM t2)", [], |r| r.get(0)).unwrap();
...
complex_test!(complex_intersect, |db: &TestDb| {
    ...
    let count: i64 = db.query_row("SELECT COUNT(*) FROM (SELECT val FROM t1 INTERSECT SELECT val FROM t2)", [], |r| r.get(0)).unwrap();
...
complex_test!(complex_except, |db: &TestDb| {
    ...
    db.execute("INSERT INTO t1 VALUES (1), (2), (3)", []).unwrap();
    db.execute("INSERT INTO t2 VALUES (2), (3), (4)", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM (SELECT val FROM t1 EXCEPT SELECT val FROM t2)", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 1); // Only 1
});

// Additional complex tests (20 more for ~30 total)
complex_test!(complex_three_table_join, |db: &TestDb| {
    db.execute("CREATE TABLE a (id INTEGER, val TEXT)", []).unwrap();
    db.execute("CREATE TABLE b (id INTEGER, a_id INTEGER, val TEXT)", []).unwrap();
    db.execute("CREATE TABLE c (id INTEGER, b_id INTEGER, val TEXT)", []).unwrap();
...
complex_test!(complex_cross_join, |db: &TestDb| {
    ...
    let count: i64 = db.query_row("SELECT COUNT(*) FROM t1 CROSS JOIN t2", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 4); // 2x2
});

complex_test!(complex_self_join, |db: &TestDb| {
    db.execute("CREATE TABLE employees (id INTEGER, name TEXT, manager_id INTEGER)", []).unwrap();
    db.execute("INSERT INTO employees VALUES (1, 'Alice', NULL), (2, 'Bob', 1), (3, 'Charlie', 1)", []).unwrap();
    let count: i64 = db.query_row(
...
complex_test!(complex_correlated_subquery, |db: &TestDb| {
    db.execute("CREATE TABLE products (id INTEGER, category TEXT, price REAL)", []).unwrap();
    db.execute("INSERT INTO products VALUES (1, 'A', 10), (2, 'A', 20), (3, 'B', 15), (4, 'B', 25)", []).unwrap();
    let count: i64 = db.query_row(
...
complex_test!(complex_exists_subquery, |db: &TestDb| {
    ...
    db.execute("CREATE TABLE orders (user_id INTEGER, amount REAL)", []).unwrap();
    db.execute("INSERT INTO users VALUES (1, 'Alice'), (2, 'Bob'), (3, 'Charlie')", []).unwrap();
    db.execute("INSERT INTO orders VALUES (1, 100), (2, 200)", []).unwrap();
    let count: i64 = db.query_row(
...
complex_test!(complex_in_subquery, |db: &TestDb| {
    db.execute("CREATE TABLE products (id INTEGER, price REAL)", []).unwrap();
    db.execute("CREATE TABLE featured (product_id INTEGER)", []).unwrap();
    db.execute("INSERT INTO products VALUES (1, 10), (2, 20), (3, 30)", []).unwrap();
    db.execute("INSERT INTO featured VALUES (1), (3)", []).unwrap();
...
complex_test!(complex_multiple_ctes, |db: &TestDb| {
    db.execute("CREATE TABLE sales (product TEXT, amount REAL)", []).unwrap();
    db.execute("INSERT INTO sales VALUES ('A', 100), ('B', 200), ('A', 50)", []).unwrap();
...
complex_test!(complex_window_over_partition, |db: &TestDb| {
    db.execute("CREATE TABLE sales (region TEXT, amount REAL)", []).unwrap();
    db.execute("INSERT INTO sales VALUES ('East', 100), ('East', 200), ('West', 150), ('West', 250)", []).unwrap();
    let sum: f64 = db.query_row(
...
complex_test!(complex_case_in_join, |db: &TestDb| {
    db.execute("CREATE TABLE orders (id INTEGER, status TEXT)", []).unwrap();
    db.execute("INSERT INTO orders VALUES (1, 'pending'), (2, 'shipped'), (3, 'pending')", []).unwrap();
    let count: i64 = db.query_row(
...
complex_test!(complex_group_by_having_order, |db: &TestDb| {
    db.execute("CREATE TABLE sales (product TEXT, amount REAL)", []).unwrap();
    db.execute("INSERT INTO sales VALUES ('A', 10), ('B', 20), ('A', 15), ('C', 5), ('B', 30)", []).unwrap();
...
complex_test!(complex_multi_column_order, |db: &TestDb| {
    ...
    db.execute("INSERT INTO t VALUES (1, 2, 100), (1, 1, 200), (2, 1, 300)", []).unwrap();
    let c: i64 = db.query_row("SELECT c FROM t ORDER BY a ASC, b DESC LIMIT 1", [], |r| r.get(0)).unwrap();
    assert_eq!(c, 100); // a=1, b=2
});

complex_test!(complex_aggregate_with_filter, |db: &TestDb| {
    db.execute("CREATE TABLE t (category TEXT, val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES ('A', 10), ('B', 20), ('A', 30), ('B', 40)", []).unwrap();
    let sum: i64 = db.query_row("SELECT SUM(val) FILTER (WHERE category='A') FROM t", [], |r| r.get(0)).unwrap();
    assert_eq!(sum, 40);
});

complex_test!(complex_join_with_aggregate, |db: &TestDb| {
    db.execute("CREATE TABLE orders (id INTEGER, customer_id INTEGER, amount REAL)", []).unwrap();
    db.execute("CREATE TABLE customers (id INTEGER, name TEXT)", []).unwrap();
    db.execute("INSERT INTO customers VALUES (1, 'Alice'), (2, 'Bob')", []).unwrap();
    db.execute("INSERT INTO orders VALUES (1, 1, 100), (2, 1, 200), (3, 2, 150)", []).unwrap();
    let total: f64 = db.query_row(
...
complex_test!(complex_subquery_scalar, |db: &TestDb| {
    db.execute("CREATE TABLE t (val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (1), (2), (3)", []).unwrap();
    let result: i64 = db.query_row("SELECT (SELECT COUNT(*) FROM t) + (SELECT MAX(val) FROM t)", [], |r| r.get(0)).unwrap();
...
complex_test!(complex_lateral_join_workaround, |db: &TestDb| {
    ...
    db.execute("CREATE TABLE t1 (id INTEGER, val INTEGER)", []).unwrap();
    db.execute("CREATE TABLE t2 (id INTEGER, t1_id INTEGER, val INTEGER)", []).unwrap();
...
complex_test!(complex_multiple_aggregates, |db: &TestDb| {
    db.execute("CREATE TABLE t (val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (10), (20), (30), (40), (50)", []).unwrap();
    let (count, sum, avg, min, max): (i64, i64, f64, i64, i64) = db.query_row(
...
complex_test!(complex_string_aggregation, |db: &TestDb| {
    db.execute("CREATE TABLE t (val TEXT)", []).unwrap();
    db.execute("INSERT INTO t VALUES ('a'), ('b'), ('c')", []).unwrap();
    let result: String = db.query_row("SELECT GROUP_CONCAT(val, ',') FROM t", [], |r| r.get(0)).unwrap();
    assert_eq!(result, "a,b,c");
});

complex_test!(complex_date_functions, |db: &TestDb| {
    db.execute("CREATE TABLE events (id INTEGER, ts TEXT)", []).unwrap();
    db.execute("INSERT INTO events VALUES (1, '2024-01-15 10:30:00'), (2, '2024-01-16 11:45:00')", []).unwrap();
    let date: String = db.query_row("SELECT DATE(ts) FROM events WHERE id=1", [], |r| r.get(0)).unwrap();
...
wal_test!(wal_checkpoint_passive, |db: &TestDb| {
    ...
    db.execute("CREATE TABLE t (val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (1), (2), (3)", []).unwrap();
    let _: (i32, i32, i32) = db.query_row("PRAGMA wal_checkpoint(PASSIVE)", [], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?))).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM t", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 3);
});
...
wal_test!(wal_checkpoint_full, |db: &TestDb| {
    ...
    let _: (i32, i32, i32) = db.query_row("PRAGMA wal_checkpoint(FULL)", [], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?))).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM t", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 100);
});
...
wal_test!(wal_checkpoint_restart, |db: &TestDb| {
    ...
    db.execute("CREATE TABLE t (val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (1)", []).unwrap();
    let _: (i32, i32, i32) = db.query_row("PRAGMA wal_checkpoint(RESTART)", [], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?))).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM t", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 1);
});
...
wal_test!(wal_checkpoint_truncate, |db: &TestDb| {
    ...
    db.execute("CREATE TABLE t (val INTEGER)", []).unwrap();
    db.execute("INSERT INTO t VALUES (1)", []).unwrap();
    let _: (i32, i32, i32) = db.query_row("PRAGMA wal_checkpoint(TRUNCATE)", [], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?))).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM t", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 1);
});
...
wal_test!(wal_autocheckpoint, |db: &TestDb| {
    let threshold: i64 = db.query_row("PRAGMA wal_autocheckpoint", [], |r| r.get(0)).unwrap();
    assert!(threshold > 0);
});
...
wal_test!(wal_set_autocheckpoint, |db: &TestDb| {
    ...
    let _: i64 = db.query_row("PRAGMA wal_autocheckpoint=5000", [], |r| r.get(0)).unwrap();
    let threshold: i64 = db.query_row("PRAGMA wal_autocheckpoint", [], |r| r.get(0)).unwrap();
...
wal_test!(wal_read_after_write, |db: &TestDb| {
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, val TEXT)", []).unwrap();
    db.execute("INSERT INTO t VALUES (1, 'initial')", []).unwrap();
...
wal_test!(wal_synchronous_normal, |db: &TestDb| {
    let sync: i64 = db.query_row("PRAGMA synchronous", [], |r| r.get(0)).unwrap();
    // 0=OFF, 1=NORMAL, 2=FULL, 3=EXTRA
    assert!(sync >= 0 && sync <= 3);
});

wal_test!(wal_busy_timeout, |db: &TestDb| {
    let timeout: i64 = db.query_row("PRAGMA busy_timeout", [], |r| r.get(0)).unwrap();
...
provider_test!(provider_create_item, |db: &TestDb| {
    db.execute("CREATE TABLE work_items (id TEXT PRIMARY KEY, status TEXT, data TEXT)", []).unwrap();
    db.execute("INSERT INTO work_items VALUES ('item-1', 'available', 'payload')", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM work_items WHERE status='available'", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 1);
});

provider_test!(provider_peek_lock, |db: &TestDb| {
    db.execute("CREATE TABLE work_items (id TEXT PRIMARY KEY, status TEXT, locked_until TEXT)", []).unwrap();
    db.execute("INSERT INTO work_items VALUES ('item-1', 'available', NULL)", []).unwrap();
    ...
    let status: String = db.query_row("SELECT status FROM work_items WHERE id='item-1'", [], |r| r.get(0)).unwrap();
    assert_eq!(status, "locked");
});

provider_test!(provider_complete_item, |db: &TestDb| {
    db.execute("CREATE TABLE work_items (id TEXT PRIMARY KEY, status TEXT)", []).unwrap();
    db.execute("INSERT INTO work_items VALUES ('item-1', 'locked')", []).unwrap();
    db.execute("UPDATE work_items SET status='completed' WHERE id='item-1' AND status='locked'", []).unwrap();
    let status: String = db.query_row("SELECT status FROM work_items WHERE id='item-1'", [], |r| r.get(0)).unwrap();
...
provider_test!(provider_abandon_item, |db: &TestDb| {
    db.execute("CREATE TABLE work_items (id TEXT PRIMARY KEY, status TEXT)", []).unwrap();
    db.execute("INSERT INTO work_items VALUES ('item-1', 'locked')", []).unwrap();
    db.execute("UPDATE work_items SET status='available' WHERE id='item-1' AND status='locked'", []).unwrap();
    let status: String = db.query_row("SELECT status FROM work_items WHERE id='item-1'", [], |r| r.get(0)).unwrap();
...
provider_test!(provider_delete_item, |db: &TestDb| {
    db.execute("CREATE TABLE work_items (id TEXT PRIMARY KEY, status TEXT)", []).unwrap();
    db.execute("INSERT INTO work_items VALUES ('item-1', 'completed')", []).unwrap();
    db.execute("DELETE FROM work_items WHERE id='item-1'", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM work_items", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 0);
});
...
provider_test!(provider_update_data, |db: &TestDb| {
    db.execute("CREATE TABLE work_items (id TEXT PRIMARY KEY, data TEXT)", []).unwrap();
    db.execute("INSERT INTO work_items VALUES ('item-1', 'old')", []).unwrap();
    db.execute("UPDATE work_items SET data='new' WHERE id='item-1'", []).unwrap();
    let data: String = db.query_row("SELECT data FROM work_items WHERE id='item-1'", [], |r| r.get(0)).unwrap();
    assert_eq!(data, "new");
});

provider_test!(provider_expire_locks, |db: &TestDb| {
    db.execute("CREATE TABLE work_items (id TEXT, status TEXT, locked_until TEXT)", []).unwrap();
...
provider_test!(provider_batch_create, |db: &TestDb| {
    ...
    db.execute("BEGIN", []).unwrap();
    for i in 0..20 {
        db.execute("INSERT INTO work_items VALUES (?, 'available')", [format!("item-{}", i)]).unwrap();
    }
    db.execute("COMMIT", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM work_items", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 20);
...
provider_test!(provider_claim_oldest, |db: &TestDb| {
    db.execute("CREATE TABLE work_items (id TEXT PRIMARY KEY, status TEXT, created_at TEXT)", []).unwrap();
    ...
    let id: String = db.query_row("SELECT id FROM work_items WHERE status='locked'", [], |r| r.get(0)).unwrap();
    assert_eq!(id, "item-1");
});

// Additional provider tests (7 more for ~17 total)
provider_test!(provider_retry_count, |db: &TestDb| {
    db.execute("CREATE TABLE work_items (id TEXT PRIMARY KEY, retry_count INTEGER)", []).unwrap();
    db.execute("INSERT INTO work_items VALUES ('item-1', 0)", []).unwrap();
    db.execute("UPDATE work_items SET retry_count=retry_count+1 WHERE id='item-1'", []).unwrap();
    let count: i64 = db.query_row("SELECT retry_count FROM work_items WHERE id='item-1'", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 1);
});

provider_test!(provider_max_retries, |db: &TestDb| {
    db.execute("CREATE TABLE work_items (id TEXT, status TEXT, retry_count INTEGER)", []).unwrap();
    db.execute("INSERT INTO work_items VALUES ('item-1', 'failed', 5), ('item-2', 'failed', 2)", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM work_items WHERE retry_count >= 3", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 1);
});

provider_test!(provider_priority_queue, |db: &TestDb| {
    db.execute("CREATE TABLE work_items (id TEXT, priority INTEGER, status TEXT)", []).unwrap();
    db.execute("INSERT INTO work_items VALUES ('item-1', 5, 'available'), ('item-2', 10, 'available'), ('item-3', 1, 'available')", []).unwrap();
    let id: String = db.query_row("SELECT id FROM work_items WHERE status='available' ORDER BY priority DESC LIMIT 1", [], |r| r.get(0)).unwrap();
    assert_eq!(id, "item-2");
});

provider_test!(provider_bulk_complete, |db: &TestDb| {
    db.execute("CREATE TABLE work_items (id TEXT, status TEXT)", []).unwrap();
    for i in 0..10 {
        db.execute("INSERT INTO work_items VALUES (?, 'locked')", [format!("item-{}", i)]).unwrap();
    }
    db.execute("UPDATE work_items SET status='completed' WHERE status='locked'", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM work_items WHERE status='completed'", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 10);
});

provider_test!(provider_message_properties, |db: &TestDb| {
    db.execute("CREATE TABLE messages (id TEXT PRIMARY KEY, body TEXT, properties TEXT)", []).unwrap();
    db.execute(r#"INSERT INTO messages VALUES ('msg-1', 'payload', '{"type":"order","priority":1}')"#, []).unwrap();
    let props: String = db.query_row("SELECT properties FROM messages WHERE id='msg-1'", [], |r| r.get(0)).unwrap();
    assert!(props.contains("order"));
});

provider_test!(provider_dequeue_count, |db: &TestDb| {
    db.execute("CREATE TABLE messages (id TEXT, dequeue_count INTEGER)", []).unwrap();
    db.execute("INSERT INTO messages VALUES ('msg-1', 0)", []).unwrap();
    db.execute("UPDATE messages SET dequeue_count=dequeue_count+1 WHERE id='msg-1'", []).unwrap();
    db.execute("UPDATE messages SET dequeue_count=dequeue_count+1 WHERE id='msg-1'", []).unwrap();
    let count: i64 = db.query_row("SELECT dequeue_count FROM messages WHERE id='msg-1'", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 2);
});

provider_test!(provider_visibility_timeout, |db: &TestDb| {
    db.execute("CREATE TABLE messages (id TEXT, status TEXT, visible_at TEXT)", []).unwrap();
    db.execute("INSERT INTO messages VALUES ('msg-1', 'invisible', DATETIME('now', '+5 minutes'))", []).unwrap();
    db.execute("INSERT INTO messages VALUES ('msg-2', 'invisible', '2020-01-01')", []).unwrap();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM messages WHERE visible_at <= DATETIME('now')", [], |r| r.get(0)).unwrap();
