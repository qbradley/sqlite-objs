...
TEST(page_blob_write_at_offset) {
    ...
    azure_err_t rc = g_ops->page_blob_write(g_ctx, "test.db", 1024, data, 512, NULL, NULL, &err);
    ASSERT_AZURE_OK(rc);
...
TEST(page_blob_write_unaligned_offset_fails) {
    ...
    uint8_t data[512];
    azure_err_t rc = g_ops->page_blob_write(g_ctx, "test.db", 100, data, 512, NULL, NULL, &err);
    ASSERT_AZURE_ERR(rc, AZURE_ERR_ALIGNMENT);
}
...
TEST(page_blob_write_unaligned_length_fails) {
    ...
    uint8_t data[100];
    azure_err_t rc = g_ops->page_blob_write(g_ctx, "test.db", 0, data, 100, NULL, NULL, &err);
    ASSERT_AZURE_ERR(rc, AZURE_ERR_ALIGNMENT);
}
...
TEST(page_blob_write_nonexistent_fails) {
    ...
    azure_error_t err;
    uint8_t data[512];
    azure_err_t rc = g_ops->page_blob_write(g_ctx, "ghost.db", 0, data, 512, NULL, NULL, &err);
...
TEST(page_blob_write_grows_blob) {
    ...
    azure_err_t rc = g_ops->page_blob_write(g_ctx, "test.db", 512, data, 512, NULL, NULL, &err);
    ASSERT_AZURE_OK(rc);
...
TEST(page_blob_write_overwrite) {
    ...
    g_ops->page_blob_write(g_ctx, "test.db", 0, data1, 512, NULL, NULL, &err);
    g_ops->page_blob_write(g_ctx, "test.db", 0, data2, 512, NULL, NULL, &err);
...
TEST(page_blob_read_at_offset) {
    ...
    g_ops->page_blob_write(g_ctx, "test.db", 1024, data, 512, NULL, NULL, &err);
...
TEST(fail_at_specific_call) {
    ...
    uint8_t data[512];
    azure_err_t rc2 = g_ops->page_blob_write(g_ctx, "test.db", 0, data, 512, NULL, NULL, &err);
...
TEST(fail_operation_always) {
    ...
    uint8_t data[512];
    azure_err_t rc1 = g_ops->page_blob_write(g_ctx, "test.db", 0, data, 512, NULL, NULL, &err);
...
TEST(call_count_per_operation) {
    ...
    uint8_t data[512] = {0};
    g_ops->page_blob_write(g_ctx, "test.db", 0, data, 512, NULL, NULL, &err);
    g_ops->page_blob_write(g_ctx, "test.db", 512, data, 512, NULL, NULL, &err);
    g_ops->page_blob_write(g_ctx, "test.db", 1024, data, 512, NULL, NULL, &err);
...
TEST(multiple_blobs_independent) {
    ...
    g_ops->page_blob_write(g_ctx, "a.db", 0, data_a, 512, NULL, NULL, &err);
    g_ops->page_blob_write(g_ctx, "b.db", 0, data_b, 512, NULL, NULL, &err);
...
TEST(page_blob_write_4096_page_size) {
    ...
    azure_err_t rc = g_ops->page_blob_write(g_ctx, "test.db", 4096, page, 4096, NULL, NULL, &err);
    ASSERT_AZURE_OK(rc);
