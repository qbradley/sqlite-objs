...
### D-CW-001: Fail-Fast on Stale Snapshot (SQLITE_BUSY)
**Date:** 2026-06-27 | **From:** Aragorn (VFS Implementation)

**Problem:** Concurrent writers to sqlite-objs lost inserts (40 expected, 17 persisted) due to ETag revalidation timing window. Connection A would commit, connection B would proceed with stale cache, and subsequent xSync would overwrite A's changes.

**Solution:** Refresh stale blobs before SQLite reads pages for a transaction (`revalidateBeforeRead()` at xLock(SHARED)). If blob changes after pages have been read under the current SHARED lock, fail with `SQLITE_BUSY` instead of re-downloading. This preserves SQLite's snapshot isolation guarantee — each transaction sees a consistent point-in-time view.

**Implementation:** Modified `src/sqlite_objs_vfs.c`:
- `xLock(SHARED)` calls `revalidateBeforeRead()` to HEAD-check blob and refresh cache before pages are read
- `sqliteObjsRead()` tracks MAIN_DB reads after current SHARED lock
- `revalidateAfterLease()` returns `SQLITE_BUSY` if ETag mismatch detected post-read
- Added `revalidation_busy` metric to track stale-snapshot write retries

**Consequences:** More frequent SQLITE_BUSY under heavy concurrent writes (clients should set `busy_timeout=5000`). Trades write latency for correctness — no silent data loss.

---
### D-CW-002: Concurrent Write Safety Contract
**Date:** 2026-06-27 | **From:** Gandalf (Architecture Diagnosis)

**Intended contract:** sqlite-objs implements **D3: Two-Level Lease-Based Locking**. Concurrent writers serialize via Azure blob lease exclusion. Only one writer can hold the 30-second lease at a time; others get `SQLITE_BUSY` (retryable).

**Root cause of bug:** Per-connection in-memory cache with no cross-connection invalidation. When Azurite's lease-acquire response lacked ETag, revalidation could race with previous writer's page uploads, proceeding with stale cache and corrupting data.

**Architectural fix:** Same-process shared ETag state within `pVfsData` eliminates race for multi-connection scenarios. Paired with SHARED-lock revalidation (D-CW-001), this enforces correct behavior across process boundaries.

---
### D-CW-003: Concurrent Writer Regression Test
**Date:** 2026-06-27 | **From:** Samwise (QA)

**Test:** `test/test_integration.c :: concurrent_writers_regression`
- Spawns 4 pthreads writing concurrently to same database
- Each writer attempts 10 inserts (40 total)
- Acceptable outcomes: all writers serialize with all 40 inserts, or some hit SQLITE_BUSY and back off
- Rejects: silent data loss, duplicate rowids

**Result:** ✅ 40 assigned IDs, 40 persisted rows, all IDs distinct. No data loss. All 42 integration tests passing (41 previous + 1 new).
